...
class ProgramOverviewViewModelTest {
    ...
    fun uiState_success_whenProgramExists() = runTest {
        ...
        val viewModel = ProgramOverviewViewModel("p1", repository)
        
        viewModel.uiState.test {
            val state = awaitItem() as ProgramOverviewUiState.Success
    ...
    fun uiState_error_whenProgramNotFound() = runTest {
        ...
        viewModel.uiState.test {
            val state = awaitItem() as ProgramOverviewUiState.Error
            assertEquals("Program not found", state.message)
        }
    }
}
